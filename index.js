...
app.get('/test', async (req, res) => {
  ...
  try {
    ...
    if (normalMatch && normalMatch[1] && normalMatch[1].length > 1000) {
      ...
      var shaped = {
        price: prsd.data.priceComponent.origPrice.minPrice,
        maxPrice: prsd.data.priceComponent.origPrice.maxPrice,
        discountPrice: prsd.data.priceComponent.discountPrice.minActMultiCurrencyPrice != undefined && prsd.data.priceComponent.discountPrice.minActMultiCurrencyPrice || "No discount Price",
        maxDiscountPrice: prsd.data.priceComponent.discountPrice.maxActMultiCurrencyPrice != undefined && prsd.data.priceComponent.discountPrice.maxActMultiCurrencyPrice || "No discount Price"
      };

      res.json(prsd.data.priceComponent)
    } else {
      res.json({ ok: false });
      console.error('Unable to extract window.runParams data.');
    }
